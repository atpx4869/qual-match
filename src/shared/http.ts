...
// 移植自 bzxz src/shared/http.ts（功能零改，注释适配单用户语境）。
...
// 维护独立 Pool）。单用户场景并发本就低，32 远超实际需要，仅作上限兜底，
// 远低于上游政府/认证站点的限流阈值。
export const httpAgent = new Agent({
  keepAliveTimeout: 30_000,
  keepAliveMaxTimeout: 60_000,
  connections: 32,
  // 每连接最多 4 个 in-flight 请求，吸收 captcha+verify+view 突发而不新开 TCP。
  pipelining: 4,
});
...
// Records min / max / mean latency per origin so a diagnostics endpoint can
// surface "上游是瓶颈" without users having to read raw logs.
