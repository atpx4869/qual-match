...
/**
 * 省级 CMA 抓取器（湖北公示库，HTTP + cheerio）。移植自 bzxz src/services/cma-scraper.ts。
 *
 * 关键事实：证书详情 + 全部能力明细在**同一个 HTML 页**里，scrapeFull 只发 1 个请求
 * （fetchDetailHtml）后对同一 html 跑 parseDetail + parseCapabilities，无分页。
 *
 * 移植改动：删 bzxz 的 checkForUpdate（查新，单用户不需要）；import 路径适配 sources/ 子目录。
 */
